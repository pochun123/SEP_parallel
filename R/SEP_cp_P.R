...
#' @param nthreads A numeric value specifying the maximum number of parallel threads to use. Default is \code{2}.
...
SepEP <- function(data, tt, B1 = NA, B2 = NA, r1 = NA, r2 = NA, rr = NA, knots = NA, M = "LT", con = "median", level = NULL, type = "right", nthreads = 2){
  ...
  if (is.na(r1)){
    ...
    lg1s <- c()
    # initialize parallel cluster with
    cl = makeCluster(nThreads)

    # sets up the cluster for the foreach loop
    registerDoParallel(cl)

    lg1s <- foreach (w = 1:length(knots), .combine = 'c') %dopar% {
      B1 <- B1
      B2 <- B2
  ...
  if (is.na(r2)){
    ...
    lg2s <- foreach (w = 1:length(knots), .combine = 'c') %dopar% {
      B1 <- B1
      B2 <- B2
